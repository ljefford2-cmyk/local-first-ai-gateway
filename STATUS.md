...
**Last documentation update:** 2026-06-04 (egress-remediation + KI-3 reconciliation pass)
**Note on currency:** "Originally generated" is the date the Spec 1–7 matrix was first produced. It is *not* the complete current truth for everything below. The entire "Phase 4A — Mobile Agent Command Harness" section and rows 4A.1–4A.25 were added on 2026-04-26 (commits `7bd5df5`, `eff74ee`, `8b3946e`, `6ed818f`, `cca7027`), and the test-count figures in the Test Coverage Summary and the closing summary were re-measured from the working tree on 2026-05-29 (the commands used are shown in those sections). Where a dated sub-entry conflicts with the top-level generation date, the dated sub-entry is authoritative. Egress network-level isolation and the dispatch-authority gate (Patch A `c038dd4`, Patch B `f956d8e`), the hostile-worker boundary probe (`ebee130`), and the KI-3 resolution (`709d8e6`) landed after 2026-05-29 and are the latest authoritative sub-entries.
...
| 4.4 | Network isolation (three Docker networks) | Implemented | `docker-compose.yml` — `drnt-internal` (service mesh), `drnt-external` (cloud egress), and `drnt-sandbox` (`internal: true`, no public route — egress-workers run here only). Orchestrator on `drnt-internal` only; `egress-gateway` and `ollama` bridge all three (Patch A, `c038dd4`). |
...
| 4.10 | Egress dispatch-authority gate | **Implemented** | `egress-gateway/main.py` — `/dispatch` requires an `Authorization: Bearer` dispatch-authority token, evaluated fail-closed BEFORE route lookup. Orchestrator presents `DRNT_DISPATCH_AUTH_TOKEN` (`docker-compose.yml`); workers never receive it, so a tokenless worker is rejected `401` before any route validation or cloud spend. Tests: `tests/test_egress_dispatch_authority.py` (Patch B, `f956d8e`). |
| 4.11 | Hostile-worker egress boundary regression guard | **Implemented** | `tests/test_e2e_v02.py::TestEgressBoundaryProbe` asserts the secure state on `drnt-sandbox`: (A) worker → external `1.1.1.1:443` blocked, (B) worker → `egress-gateway` `/dispatch` reachable-but-`401` (Patch B gate), (C) worker → `ollama:11434` reachable (control). Harness: `orchestrator/admin_routes.py` (`POST /admin/e2e/egress-probe`), `worker/worker_agent.py:handle_egress_probe`. `@pytest.mark.e2e`, stack-gated (`ebee130`). |
...
| 6.12 | Network isolation enforced per-worker | **Implemented** | Blueprint-driven network isolation: workers with empty `egress_allow` get `network_mode="none"` (no network); workers with egress endpoints connect to `drnt-sandbox` (`blueprint_engine.py:_build_network_config()`). Default is no-network (fail closed). The code-level `EgressProxy` allowlist is now backed by network-level containment — `drnt-sandbox` is `internal: true`, so a compromised worker cannot reach the internet directly (Patch A, `c038dd4`). Worker-proxy validator rejects `network_mode` values other than the registry's `allowed_networks` (`["drnt-sandbox"]`) or unset (Phase 2B). |
...
| 6.15 | Worker external egress blocked at network layer | **Implemented** | Beyond the code-level proxy allowlist (row 6.12), worker external egress is contained at the network layer: `drnt-sandbox` is `internal: true` (`docker-compose.yml`), so a worker cannot open direct outbound internet connections — only in-mesh services (`ollama`, `egress-gateway`) are reachable. Live-verified and regression-guarded (row 4.11) (Patch A, `c038dd4`). |
...
§6B gives a mobile **source event** `(client_source, client_source_event_id)` a durable, intent-aware identity so that re-sending the same source event is deduplicated and a source event re-sent with **changed intent** is blocked with an explicit interim `409` instead of silently creating a second executing job. This closes the gaps the [Source-Event Identity current-truth report](DRNT_Week1_Phase4A2_Evidence/current_truth_report.md) proved live: `client_source_event_id` was accepted then dropped (no persistence, no audit, no dedup), and the only durable submit guard (`idempotency_key`) is purged for terminal jobs on restart. Scope here is **§6B-1 + §6B-2 (interim)**; the §6B-3 human-reconfirmation hold is deferred and the `409` is explicitly marked interim. See [`docs/plans/6b-source-event-identity.md`](docs/plans/6b-source-event-identity.md). The "§6B" label is the Source-Event Identity ADR's section number — **not** the repo's Spec-6 `phase6b` (Silo Runtime Security).

| # | Claim | Status | Evidence |
|---|-------|--------|----------|
| 6B.1 | Deterministic `normalized_raw_input` + intent-equivalence hash over exactly `{normalized_raw_input, input_modality}` | **Implemented** | `orchestrator/source_event.py:normalize_raw_input` (NFC + collapse internal whitespace runs to a single ASCII space + trim; case and punctuation preserved) and `compute_intent_equivalence_hash` (SHA-256 over canonical JSON). `device`, `client_timestamp`, `idempotency_key`, `client_source` do **not** participate. Tested in `orchestrator/test_source_event.py` (17 cases: whitespace-only/leading-trailing invariance, NFC equivalence, modality/raw-input sensitivity, device/timestamp invariance-by-construction). |
| 6B.2 | Durable source-event identity namespace `srcevent:`, never purged, DB-arbitrated first-writer | **Implemented** | `orchestrator/idempotency_store.py:SourceEventRecord`, `check_and_store_source_event` (`new`/`equivalent`/`conflict`), `release_source_event` (compare-and-delete rollback), `get_source_event` (memory→DB); loaded on startup and **not** matched by `_purge_terminal_keys` (which matches only `idem:`). First-writer is arbitrated by SQLite via `INSERT … ON CONFLICT(key) DO NOTHING` (win/lose by `total_changes` delta; on lose the canonical DB row is read and hashes compared) — **not** the prior last-writer `INSERT OR REPLACE`. Keyed `srcevent:{client_source}:{client_source_event_id}`. Durability proven in `tests/test_source_event_identity.py::test_equivalent_replay_after_terminal_and_restart_returns_original`; DB first-writer + rollback in `::test_db_reserve_source_event_is_first_writer_wins`, `::test_release_source_event_compare_and_delete`. |
| 6B.3 | Mobile lineage persisted on `Job` and in the `job.submitted` payload (closes Phase 4A AC#6) | **Implemented** | `orchestrator/models.py:Job` gains `client_source`, `client_source_event_id`, `client_timestamp`, `intent_equivalence_hash` (auto-persist via `asdict`/`Job(**data)`). `orchestrator/events.py:event_job_submitted` payload preserves all four; the envelope `source`/`source_event_id`/`timestamp` stay server-generated (no audit-dedup collision). Tested in `tests/test_source_event_identity.py::test_lineage_persisted_on_job_and_submitted_event`, `::test_backward_compat_no_client_fields_keeps_server_envelope_and_no_mapping`. |
| 6B.4 | Equivalent replay dedups to the original job (incl. different idem key, envelope drift, terminal + restart); race-safe under concurrent retries | **Implemented** | `orchestrator/job_manager.py:submit_job` source-event guard, authoritative over `idempotency_key`; equivalent replay returns the original job (`_load_job_by_id` point-reads a terminal original). A **per-source-event `asyncio.Lock`** (`_source_event_lock`) serializes same-key submits across the whole materialization path, and the `srcevent:` mapping is committed first-writer with **rollback on pre-persist failure**, so an equivalent replay cannot observe an unmaterialized mapping. Tested in `::test_equivalent_replay_dedups_across_whitespace_and_different_idem_key`, `::test_equivalent_replay_survives_envelope_drift`, `::test_equivalent_replay_after_terminal_and_restart_returns_original`, `::test_concurrent_identical_submits_create_one_job`. |
| 6B.5 | Changed-intent replay → interim `409` + durable conflict audit event; no duplicate job, mapping preserved | **Implemented (interim)** | `submit_job` raises `SourceEventIntentConflict` after emitting `orchestrator/events.py:event_source_event_conflict` (`source_event.conflict`); creates no job, enqueues no classification, does not overwrite the mapping. Body conveys `source_event_replay`, `source_event_conflict`, `conflict_type=intent_mismatch`, `original_job_id`, `action_required=reconfirmation_required`, `adr_compliance=partial_until_reconfirmation_path_exists`. Tested in `tests/test_source_event_identity.py::test_changed_intent_raises_conflict_with_locked_body`, `::test_changed_intent_blocks_duplicate_and_preserves_mapping_and_audits`. Interim until **6B.7**. |
| 6B.6 | Submit route returns `409` for changed intent (never `202`) and `503` for consistency-pending | **Implemented (interim)** | `orchestrator/main.py:submit_job` forwards client fields and catches `SourceEventIntentConflict` → `409` and `SourceEventConsistencyPending` → `503` ahead of its broad handler. Tested in `tests/test_source_event_identity.py::test_route_returns_409_for_changed_intent_not_202`, `::test_route_returns_503_for_consistency_pending`; live local-model coverage in `tests/test_source_event_identity_e2e.py::test_changed_intent_replay_returns_409`. |
| 6B.7 | §6B-3 human reconfirmation / reviewable-ambiguity hold | **Not implemented (deferred)** | Out of scope for this slice. Final ADR compliance requires a reconfirmation path; the `409` is interim (`adr_compliance=partial_until_reconfirmation_path_exists`). No code or test in this slice. |
| 6B.8 | Invariant: an existing source-event mapping blocks duplicate execution — fall-through prohibited | **Implemented** | The equivalent branch never falls through to create work. If the mapping exists but its job is not loadable (concurrent cross-process writer still materializing, or external deletion), `submit_job` briefly polls (`_await_job_loadable`), emits a best-effort `source_event.consistency_pending` diagnostic, and raises `SourceEventConsistencyPending` (retryable `503`) — creating no job. Pre-persist materialization failure (e.g. audit down) rolls back the reservation so a retry is not poisoned. Tested in `tests/test_source_event_identity.py::test_unresolvable_equivalent_mapping_fails_closed_no_duplicate`, `::test_audit_failure_rolls_back_reservation_so_retry_creates_job`. |

**Concurrency posture:** the orchestrator runs single-process (`uvicorn … ` with no `--workers`, one container). The `srcevent:` write is now first-writer (`ON CONFLICT DO NOTHING`) rather than last-writer, so it is safe should the deployment ever scale out; in-process race-safety is provided by the per-source-event async lock + reserve-with-rollback (6B.4/6B.8). Full cross-process *job* dedup (vs. *mapping* integrity) would additionally require reserve-before-materialize across processes and is not needed for the current single-process model.

**Cost-aware test buckets (Insert D):** the **free** bucket (`orchestrator/test_source_event.py` 17 + `tests/test_source_event_identity.py` 15 = 32 tests) runs in-process with no model and **passed** 2026-06-01 (`pytest --noconftest`; the 32 plus the prior in-process regression set = 391 passed, 0 failed). The **local-model** bucket (`tests/test_source_event_identity_e2e.py`, 3 tests, `@pytest.mark.e2e`) drives the real pipeline to `proposal_ready`/`delivered` and remains **paused — authored but not executed** pending the concurrency hardening (now landed) and an orchestrator image rebuilt from this branch plus the local model (`llama3.1:8b`, zero cloud).
...
| §6B (Source-Event Identity) | `orchestrator/test_source_event.py`, `tests/test_source_event_identity.py` (free, in-process; incl. concurrency hardening) · `tests/test_source_event_identity_e2e.py` (local-model, `@pytest.mark.e2e`) | Free bucket: 32 passed 2026-06-01; local-model bucket authored, paused (not yet run) |
| **Total** | **45 test files** (42 as of 2026-05-29 + 3 §6B files added 2026-06-01) | |
...
### KI-3 — `cancel`/`redirect`/`escalate` late-guard TOCTOU on non-`proposal_ready` states [RESOLVED 2026-06-01] (Spec 5 row 5.3, 5.5)

**Status:** Resolved 2026-06-01 (commit `709d8e6`). The early-guard pattern from KI-2 was extended to the `cancel`/`redirect`/`escalate` branches: each now reserves `job.override_type` synchronously before its first durable emit, closing the late-guard window. A red/green concurrency test landed in `tests/test_phase5d.py`. The original analysis below is retained as audit trail. Surfaced by the KI-2 Part 1 discovery; originally out of scope for the KI-2 fix, which was scoped to the `modify` branch.
