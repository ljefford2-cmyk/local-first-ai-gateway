# DRNT v0.2.2 — Threat Model and Non-Goals
...
**Internal network ↔ External network** — Three Docker networks defined in `docker-compose.yml`: `drnt-internal` (bridge, service mesh), `drnt-external` (bridge, cloud egress), and `drnt-sandbox` (`internal: true`, no route to the public internet — egress-workers run here only, Patch A). The orchestrator sits on `drnt-internal` only. The egress gateway bridges all three; `ollama` is reachable on the service-mesh and sandbox networks (local inference + model pulls). `drnt-internal` and `drnt-external` are non-internal networks by necessity (host-port publishing, model pulls); only `drnt-sandbox` is `internal: true`.

**Gateway ↔ Cloud providers** — API keys are bind-mounted read-only from the host (`./secrets:/var/drnt/secrets:ro`). All cloud requests go through the egress gateway. The egress proxy in `egress_proxy.py` enforces per-capability allowlists at the code level. For egress-workers this code-level authorization is now backed by network-level containment — they run on `drnt-sandbox` (`internal: true`), so a compromised worker has no route to the public internet (Patch A; see item 6).
...
**5. Worker execution-plane enforcement** — The control plane (`manifest_validator.py`, `blueprint_engine.py`, `egress_proxy.py`) and execution plane are both implemented. `worker_executor.py` creates one-shot Docker containers via the `worker-proxy` sidecar HTTP API with full sandbox constraints (cap-drop ALL, read-only rootfs, no-new-privileges, memory/PID limits, seccomp profile). The sidecar also runs garbage collection for orphaned stopped containers. STATUS.md rows 6.8–6.12 are all Implemented. Network-level egress enforcement for workers is now in place: egress-workers run on `drnt-sandbox` (`internal: true`), and the egress-gateway `/dispatch` route requires the Patch B authority token that workers never hold (see item 6). Residual non-goal: isolating non-worker `drnt-internal` egress remains deferred.

**6. Network-level egress enforcement** — Now enforced for workers (Patch A). `egress_proxy.py` remains a code-level *authorization* gate (it decides which endpoints a worker may use), and it is now backed by network-level containment: egress-workers run on `drnt-sandbox`, an `internal: true` Docker network with no route to the public internet. A compromised worker can therefore no longer bypass the proxy with direct HTTP/IP calls — only `ollama` and the `egress-gateway` (both also attached to `drnt-sandbox`) are reachable, and the gateway's `/dispatch` still requires the Patch B authority token that workers never hold. The `egress_proxy.py` docstring's "true network proxy is v2" note describes that module itself; the worker-bypass threat it warned about is mitigated by the network layer. Residual non-goal: the orchestrator and other `drnt-internal` services retain NAT egress (they publish host ports and pull models), so they sit on a non-internal network by necessity; isolating non-worker direct egress is deferred.

**7. Sensitivity regex brittleness** — `context_packager.py` PII detection uses regex patterns from `config/sensitivity.json`. These are bypassable with encoding, misspelling, or novel PII formats. No Unicode normalization, no homoglyph detection. Mitigation path: Deferred (v0.3+): consider NER-based detection. V1 accepts regex as a first-pass filter, not a guarantee.

**8. Multi-hub failover** — `hub_state.py:HubStateManager` supports the state machine (ACTIVE/SUSPENDED/AWAITING_AUTHORITY) but there is no multi-hub discovery, no shared state backend, no automatic failover. The system assumes exactly one hub. The module docstring states: "Exactly one hub is authoritative at any time." Mitigation: accepted for V1 single-user scope.
